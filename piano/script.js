// 1. Initialize the Sampler
// We use a free, high-quality set of piano samples hosted on GitHub (Salamander Piano)
const sampler = new Tone.Sampler({
    urls: {
        "A0": "A0.mp3",
        "C1": "C1.mp3",
        "D#1": "Ds1.mp3",
        "F#1": "Fs1.mp3",
        "A1": "A1.mp3",
        "C2": "C2.mp3",
        "D#2": "Ds2.mp3",
        "F#2": "Fs2.mp3",
        "A2": "A2.mp3",
        "C3": "C3.mp3",
        "D#3": "Ds3.mp3",
        "F#3": "Fs3.mp3",
        "A3": "A3.mp3",
        "C4": "C4.mp3",
        "D#4": "Ds4.mp3",
        "F#4": "Fs4.mp3",
        "A4": "A4.mp3",
        "C5": "C5.mp3",
        "D#5": "Ds5.mp3",
        "F#5": "Fs5.mp3",
        "A5": "A5.mp3",
        "C6": "C6.mp3",
        "D#6": "Ds6.mp3",
        "F#6": "Fs6.mp3",
        "A6": "A6.mp3",
        "C7": "C7.mp3",
        "D#7": "Ds7.mp3",
        "F#7": "Fs7.mp3",
        "A7": "A7.mp3",
        "C8": "C8.mp3"
    },
    // This is the "base URL" where the files are located
    baseUrl: "https://tonejs.github.io/audio/salamander/",
    
    // Callback when all sounds are loaded
    onload: () => {
        document.querySelector('.status').innerText = "Ready to Play!";
    }
}).toDestination();

// 2. Play Note Function
function playNote(note) {
    // Trigger the note immediately
    // Tone.js automatically pitches the sample if the exact file isn't found
    sampler.triggerAttackRelease(note, "2n");
}

// 3. Handle Click Events
const keys = document.querySelectorAll('.key');
keys.forEach(key => {
    key.addEventListener('mousedown', () => {
        const note = key.getAttribute('data-note');
        playNote(note);
        
        // Ensure audio context is started (browsers require user interaction)
        if (Tone.context.state !== 'running') {
            Tone.start();
        }
    });
});

// 4. Handle Keyboard Events
window.addEventListener('keydown', (e) => {
    // Prevent repeating if key is held down
    if (e.repeat) return;

    const key = document.querySelector(`.key[data-key="${e.key.toLowerCase()}"]`);
    if (key) {
        key.classList.add('active');
        const note = key.getAttribute('data-note');
        playNote(note);

        if (Tone.context.state !== 'running') {
            Tone.start();
        }
    }
});

window.addEventListener('keyup', (e) => {
    const key = document.querySelector(`.key[data-key="${e.key.toLowerCase()}"]`);
    if (key) {
        key.classList.remove('active');
    }
});
