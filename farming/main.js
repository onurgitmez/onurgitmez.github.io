...
const Game = {
    ...
    getModifiedPrice(key) {
        ...
        if (Object.values(GAME_DATA.machines).some(m => m.output === key) && this.data.upgrades.artisan_crafting) price *= 1.40;
    ...
    loadGame() {
        ...
        if (s) {
            try { this.data = { ...this.data, ...JSON.parse(s) }; this.processOffline(Date.now() - this.data.lastSave); } catch(e){}
        }
    },
    processOffline(ms) {
        if (ms < 60000) return; 
        let gained = {}, xp = 0, now = Date.now();
        const sim = (slots) => slots.forEach(slot => {
            if (!slot) return;
            const config = this.getConfig(slot.type);
            const req = this.getModifiedTime(config);
            const cycles = Math.floor((ms + (this.data.lastSave - slot.startTime)) / req);
            if (cycles > 0) {
                gained[config.output || slot.type] = (gained[config.output || slot.type] || 0) + cycles;
                xp += config.xp * cycles; slot.startTime = now - ((ms + (this.data.lastSave - slot.startTime)) % req);
            }
        });
        sim(this.data.plots.filter(p => p && GAME_DATA.trees[p.type])); sim(this.data.animals);
        for (let k in gained) this.addItem(k, gained[k]);
        if (xp > 0) this.addXp(xp);
        if (Object.keys(gained).length > 0) {
            document.getElementById('offline-report').innerHTML = Object.entries(gained).map(([k,v]) => `<div>+${v} ${k.replace('_',' ')}</div>`).join('') + `<div style="color:#2ecc71">+${xp} XP</div>`;
