...
export function PokeImg({id, size=64, style={}, shiny=false, rarity}) {
  let cls = "pixel-img";
  if (rarity === 'legendary') cls += " pixel-img-legendary";
  if (shiny) cls += " pixel-img-shiny";
  return <img src={spriteUrl(id, shiny)} alt="" className={cls} style={{ width: size, height: size, ...style }} loading="lazy" />;
}
