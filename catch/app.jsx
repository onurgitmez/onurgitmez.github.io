...
import { Home, Biomes, Encounter, Pokedex, Shop, Bag } from './screens.jsx';
...
export function App() {
  const [state, updateStateFn] = useState(() => {
    ...
    if (saved) {
      try {
        ...
        // Robust Migration for old saves
        if (parsed.caught) {
  ...
  useEffect(() => {
    ...
    localStorage.setItem("pokeCatcherSave", JSON.stringify({ money, inv, caught, seen, lure, smoke, shinyLureActive: shinyLureActive || 0 }));
  ...
  const encounter = () => {
    ...
    const isShiny = Math.random() < ((state.shinyLureActive || 0) > 0 ? 0.05 : 0.005);
    ...
    updateState({ 
      ...
      shinyLureActive: Math.max(0, (state.shinyLureActive || 0) - 1),
  ...
  const throwBall = () => {
    ...
    if (!state.inv[state.ball] || state.inv[state.ball] <= 0) { updateState({ msg: "No balls of that type!" }); return; }
    ...
    ref.current = setTimeout(() => {
      if (isCaught) {
        let currentCaught = state.caught[state.wild.id] || { normal: { C: 0, B: 0, A: 0, 'A+': 0, S: 0 }, shiny: { C: 0, B: 0, A: 0, 'A+': 0, S: 0 } };
        if (typeof currentCaught === 'number') currentCaught = { normal: { C: currentCaught, B: 0, A: 0, 'A+': 0, S: 0 }, shiny: { C: 0, B: 0, A: 0, 'A+': 0, S: 0 } };
        
        const variant = state.wildShiny ? 'shiny' : 'normal';
        const tier = state.wildTier || 'C';
        
        const newCaughtRecord = {
          ...currentCaught,
          [variant]: {
            ...currentCaught[variant],
            [tier]: (currentCaught[variant][tier] || 0) + 1
          }
        };
        
        updateState({ anim: "caught", caught: { ...state.caught, [state.wild.id]: newCaughtRecord }, msg: `✨ Gotcha! ${state.wildShiny ? 'Shiny ' : ''}${state.wild.name} (${tier} Tier) was caught!` });
        ref.current = setTimeout(() => updateState({ wild: null, anim: null, item: null }), 1800);
      } else {
        const isFled = state.smoke > 0 ? false : Math.random() < (FR[state.wild.rarity] || 0.1);
        if (isFled) { 
  ...
  const useItem = (id) => {
    if (id === "lure") { if ((state.inv.lure||0) <= 0) return; updateState({ inv: { ...state.inv, lure: state.inv.lure - 1 }, lure: state.lure + 3, msg: "🧲 Lure active!" }); }
    else if (id === "repel_flee") { if ((state.inv.repel_flee||0) <= 0) return; updateState({ inv: { ...state.inv, repel_flee: state.inv.repel_flee - 1 }, smoke: state.smoke + 3, msg: "💨 Smoke Ball active!" }); }
    else if (id === "shiny_lure") { if ((state.inv.shiny_lure||0) <= 0) return; updateState({ inv: { ...state.inv, shiny_lure: state.inv.shiny_lure - 1 }, shinyLureActive: (state.shinyLureActive||0) + 3, msg: "✨ Shiny Charm active!" }); }
  ...
  const sell = (pid, variant, tier, amount) => { 
    ...
    if (!currentCaught || !currentCaught[variant] || currentCaught[variant][tier] < amount) return; 
    
    const pokemon = POKEMON_DB.find(x => x.id === parseInt(pid)); 
    if(!pokemon) return;

    const tierMult = { 'C': 1, 'B': 1.5, 'A': 2, 'A+': 3, 'S': 5 }[tier] || 1;
  ...
  // Evolve function now explicitly targets the chosen tab's variant
  const evolve = (pid, evoData, variant = 'normal') => {
    if (state.money < evoData.cost) { alert("Not enough money to evolve!"); return; }
    const currentCaught = state.caught[pid];
    if (!currentCaught || typeof currentCaught === 'number') return;
    
    let consumedTier = null;
    
    // Check if player has the specific variant required
    for (const t of ['C', 'B', 'A', 'A+', 'S']) {
       if (currentCaught[variant] && currentCaught[variant][t] > 0) {
           consumedTier = t;
           break;
       }
    }
    
    if (!consumedTier) { alert(`You don't have a ${variant} variant of this Pokémon to evolve!`); return; }

    const newCaughtBase = { ...currentCaught, [variant]: { ...currentCaught[variant], [consumedTier]: currentCaught[variant][consumedTier] - 1 } };
    const targetId = evoData.to;
    const targetCaught = state.caught[targetId] || { normal: { C: 0, B: 0, A: 0, 'A+': 0, S: 0 }, shiny: { C: 0, B: 0, A: 0, 'A+': 0, S: 0 } };
    const newCaughtTarget = { ...targetCaught, [variant]: { ...targetCaught[variant], [consumedTier]: (targetCaught[variant][consumedTier] || 0) + 1 } };

    updateState({ money: state.money - evoData.cost, caught: { ...state.caught, [pid]: newCaughtBase, [targetId]: newCaughtTarget }, seen: { ...state.seen, [targetId]: true } });
  };
      ...
      <div style={CS.hdr}>
        ...
        <div style={CS.nav}>
          {[["home", "🏠 Home"], ["biomes", "🌍 Explore"], ["pokedex", "📖 Dex"], ["shop", "🏪 Shop"], ["bag", "🎒 Bag"]].map(([sc, ic]) => (
            <button key={sc} className="nav-btn" style={CS.nb(state.screen === sc)} onClick={() => updateState({ screen: sc, wild: null, anim: null, msg: null })}>{ic}</button>
          ))}
        </div>
      </div>
