...
import { PokeImg, PokeBallIcon } from './components.jsx';
import { CS } from './styles.js';
import { catchRate } from './utils.js';

// Tier helpers
const TIER_ORDER = ['C','B','A','A+','S'];
const tierRank = (t) => {
  const i = TIER_ORDER.indexOf(t);
  return i === -1 ? -1 : i;
};

// Highest tier owned for a given variant (normal/shiny). Returns null if no tiered data.
const getHighestTier = (caughtRecord, variant) => {
  if (!caughtRecord || typeof caughtRecord === 'number') return null;
  const v = caughtRecord?.[variant];
  if (!v) return null;
...
// Total owned of BOTH types (for Home Screen)
const getOwnedCount = (caughtRecord) => {
  if (!caughtRecord) return 0;
  if (typeof caughtRecord === 'number') return caughtRecord;
  let total = 0;
  if (caughtRecord.normal) total += Object.values(caughtRecord.normal).reduce((a, b) => a + (Number(b)||0), 0);
  if (caughtRecord.shiny) total += Object.values(caughtRecord.shiny).reduce((a, b) => a + (Number(b)||0), 0);
  return total;
};

// Total owned of a SPECIFIC variant (for Dex & Shop tabs)
const getVariantCount = (caughtRecord, variant) => {
  if (!caughtRecord) return 0;
  if (typeof caughtRecord === 'number') return variant === 'normal' ? caughtRecord : 0;
  if (!caughtRecord[variant]) return 0;
  return Object.values(caughtRecord[variant]).reduce((a, b) => a + (Number(b)||0), 0);
};
...
export function Home({state, updateState, hardReset}) {
  ...
  return (
    <div style={CS.ct} className="fade-in">
      ...
      {(state.shinyLureActive||0)>0&&<div style={{...CS.cd,background:"rgba(168,85,247,0.1)", border:"1px solid rgba(168,85,247,0.2)"}}>✨ Shiny Charm Active — {state.shinyLureActive} encounters left</div>}
    ...
    <div style={CS.ct} className="fade-in">
      <div style={{...CS.cd,textAlign:"center",position:"relative",overflow:"hidden",background:`linear-gradient(180deg,${BIOMES[state.biome]?.color}55, rgba(15,23,42,0.8))`}}>
        ...
        <div style={{height: 160, display:"flex", alignItems:"center", justifyContent:"center"}}>
          {!isShaking&&!isCaught&&<div style={{opacity:isFled?0:1,transition:"all 0.4s",animation:isFled?"none":"bounce 2s ease-in-out infinite"}}><PokeImg id={wildPokemon.id} size={140} rarity={wildPokemon.rarity} shiny={state.wildShiny} animated={true} /></div>}
        ...
        <div style={{display:"flex",gap:8,justifyContent:"center",marginBottom:16,flexWrap:"wrap"}}>
          ...
          <span style={{padding:"4px 12px",borderRadius:12,background:"rgba(168, 85, 247, 0.15)",border:"1px solid rgba(168, 85, 247, 0.4)",fontSize:12,fontWeight:800,color:"#c084fc"}}>Tier: {state.wildTier || 'C'}</span>
          {ownedCount > 0 ? (
      ...
      {!isShaking&&!isCaught&&!isFled&&<div className="fade-in">
        ...
        <div style={{display:"flex",gap:12,justifyContent:"center",marginTop:8}}>
          <button className="btn" style={{...CS.bt("#ef4444"), padding:"14px 40px", fontSize:16, boxShadow:"0 4px 14px rgba(239,68,68,0.4)"}} onClick={throwBall} disabled={!state.inv[state.ball] || state.inv[state.ball]<=0}>🎯 Throw!</button>
...
export function Pokedex({state, evolve}) {
  ...
  const [dexVariant, setDexVariant] = useState("normal"); // NEW: Pokedex Variant Tab

  const totalCaught = Object.keys(state.caught).filter(id => getOwnedCount(state.caught[id]) > 0).length;
  const totalPokemon = POKEMON_DB.length;

  const filteredList = POKEMON_DB.filter(p => {
    // Only check if you own the specific variant you are currently looking at!
    const owned = getVariantCount(state.caught[p.id], dexVariant);
    const seen = state.seen[p.id];

    if(filter === "caught" && owned === 0) return false;
    if(filter === "missing" && owned > 0) return false;
    if(filter === "seen" && (!seen || owned > 0)) return false;
  ...
  return (
    <div style={{...CS.ct, maxWidth: 800}} className="fade-in">
      ...
      {/* NEW: Toggle between Normal and Shiny Pokédex */}
      <div style={{display:"flex",gap:8,marginBottom:16, background: "rgba(0,0,0,0.2)", padding: 6, borderRadius: 12}}>
        <button className="nav-btn" style={{...CS.nb(dexVariant==="normal"), flex: 1}} onClick={()=>setDexVariant("normal")}>Normal Form</button>
        <button className="nav-btn" style={{...CS.nb(dexVariant==="shiny"), flex: 1, color: dexVariant==="shiny"?"#facc15":"#cbd5e1"}} onClick={()=>setDexVariant("shiny")}>✨ Shiny Form</button>
      </div>
        ...
        {ALL_TYPES.map(t=>(
          <button
            ...
            style={{
              ...
              padding:"4px 10px",
              borderRadius:12,
              border:typeFilter===t?"2px solid #fff":"2px solid transparent",
              background:TC[t]||"#888",
              color:"#fff",
              fontWeight:800,
              cursor:"pointer",
              opacity:typeFilter===t?1:0.6,
              boxShadow: typeFilter===t?"0 0 8px rgba(255,255,255,0.4)":""
            }}
          >
            {t}
          </button>
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(120px,1fr))",gap:12}}>
        {filteredList.map(p=>{
          const ownCount = getVariantCount(state.caught[p.id], dexVariant);
          const seen = state.seen[p.id];
          const evoData = typeof EVOLUTIONS !== 'undefined' ? EVOLUTIONS[p.id] : null;
          const isShiny = dexVariant === 'shiny';
          const highestTier = getHighestTier(state.caught[p.id], dexVariant);
          ...
          return(
            <div
              ...
              style={{
                ...CS.cd,
                padding:"12px 8px",
                opacity:ownCount?1:seen?0.7:0.3,
                textAlign:"center",
                position:"relative",
                marginBottom:0,
                background:ownCount?`linear-gradient(180deg, ${TC[p.type]}22, rgba(255,255,255,0.02))`:"rgba(255,255,255,0.02)"
              }}
            >
              ...
              <div style={{height:60,display:"flex",alignItems:"center",justifyContent:"center", margin:"4px 0"}}>
                {seen?<PokeImg id={p.id} size={56} rarity={p.rarity} shiny={isShiny}/>:<span style={{fontSize:32,filter:"grayscale(1) opacity(0.5)"}}>❓</span>}
              </div>
              <div style={{fontSize:12,fontWeight:800,lineHeight:1.2, color:seen?"#fff":"#64748b"}}>{seen?p.name:"???"}</div>

              {/* Tags row: Type + Rarity + Highest Tier (inline, near the tags you mentioned) */}
              {seen&&(
                  ...
                  {ownCount>0 && highestTier && (
                    <span
                      style={{
                        ...
                        fontSize:9,
                        fontWeight:900,
                        background: isShiny ? "rgba(250,204,21,0.14)" : "rgba(148,163,184,0.14)",
                        border: isShiny ? "1px solid rgba(250,204,21,0.35)" : "1px solid rgba(148,163,184,0.28)",
                        color: isShiny ? "#facc15" : "#e2e8f0"
                      }}
                      title={`Highest tier owned: ${highestTier}`}
                    >
                      {highestTier}
                    </span>
                  )}
              ...
              {seen&&(
                <div style={{position:"absolute",top:6,left:6,fontSize:12,opacity:0.8}} title={BIOMES[p.biome].name}>
                  {BIOMES[p.biome]?.emoji}
                </div>
              )}
              ...
              {ownCount > 0 && evoData && (
                <button
                  className="btn"
                  onClick={(e) => { e.stopPropagation(); evolve(p.id, evoData, dexVariant); }}
                  style={{marginTop: 8, ...CS.bt("#3b82f6"), fontSize: 10, padding: "4px 8px", width: "100%"}}
                >
                  Evolve (₽{evoData.cost})
                </button>
              )}
...
export function Shop({state, updateState, buy, sell}) {
  const [shopCategory, setShopCategory] = useState("balls");
  const [sellVariant, setSellVariant] = useState("normal"); // NEW: Shop variant tab

  // Filter caughtList to ONLY show Pokemon that have > 1 of the specific currently selected variant
  const caughtList = Object.entries(state.caught).filter(([id, c]) => {
    return getVariantCount(c, sellVariant) > 1;
  });
  ...
  return (
    <div style={CS.ct} className="fade-in">
      ...
      {!state.sell ? <>
        ...
        {/* NEW: Toggle between selling Normal and Shiny tiers */}
        <div style={{display:"flex",gap:8,marginBottom:16, background: "rgba(0,0,0,0.2)", padding: 6, borderRadius: 12}}>
          <button className="nav-btn" style={{...CS.nb(sellVariant==="normal"), flex: 1}} onClick={()=>setSellVariant("normal")}>Sell Normal Tiers</button>
          <button className="nav-btn" style={{...CS.nb(sellVariant==="shiny"), flex: 1, color: sellVariant==="shiny"?"#facc15":"#cbd5e1"}} onClick={()=>setSellVariant("shiny")}>✨ Sell Shiny Tiers</button>
        </div>
        ...
        {caughtList.length === 0
          ?<div style={{...CS.cd,textAlign:"center",color:"#64748b",fontWeight:700,padding:"40px 20px"}}>No {sellVariant} duplicates available to sell.<br/><span style={{fontSize:12,fontWeight:500}}>Go catch some more!</span></div>
          :<div style={{display:"grid",gap:12}}>
            {caughtList.map(([id, counts]) => {
              const pokemon = POKEMON_DB.find(p => p.id === parseInt(id));
              if(!pokemon) return null;

              const highestTier = getHighestTier(counts, sellVariant);

              const renderTierButtons = (variant, mult) => TIER_ORDER.map(tier => {
                if (!counts[variant]) return null;
                const count = counts[variant][tier] || 0;
                if (count === 0) return null;

                // Safety: never allow selling your last copy of the *highest tier you own*.
                // Lower tiers are always sellable (since you still retain the higher-tier copy).
                const isHighest = highestTier && tierRank(tier) === tierRank(highestTier);
                const sellableAmount = isHighest ? Math.max(0, count - 1) : count;
              ...
              return (
                <div key={id} className="card-hover" style={{...CS.cd, padding:"16px", marginBottom:0}}>
                  <div style={{display:"flex",alignItems:"center",gap:12, marginBottom: 12}}>
                    <PokeImg id={pokemon.id} size={48} rarity={pokemon.rarity} shiny={sellVariant === 'shiny'} />
                    <div>
                  ...
                  <div>
                    {/* Only render the buttons for the currently selected tab! */}
                    {renderTierButtons(sellVariant, sellVariant === 'shiny' ? 10 : 1)}
                  </div>
                </div>
              );
            })}
  ...
  return (
    <div style={CS.ct} className="fade-in">
      ...
      <div style={CS.cd}>
        ...
        {(state.shinyLureActive||0)>0&&<div style={{background:"rgba(168,85,247,0.1)", border:"1px solid rgba(168,85,247,0.2)", padding:"10px 16px", borderRadius:10, marginBottom:16, fontWeight:700, color:"#c084fc", fontSize:13}}>✨ Shiny Charm is active! ({state.shinyLureActive} encounters left)</div>}

        <div style={{display:"flex",gap:12,flexWrap:"wrap"}}>
          <button className="btn" style={CS.bt("#a855f7",(state.inv.lure||0)<=0)} onClick={()=>useItem("lure")} disabled={(state.inv.lure||0)<=0}>🧲 Use Lure ({state.inv.lure||0})</button>
          <button className="btn" style={CS.bt("#0ea5e9",(state.inv.repel_flee||0)<=0)} onClick={()=>useItem("repel_flee")} disabled={(state.inv.repel_flee||0)<=0}>💨 Use Smoke ({state.inv.repel_flee||0})</button>
          <button className="btn" style={CS.bt("#eab308",(state.inv.shiny_lure||0)<=0)} onClick={()=>useItem("shiny_lure")} disabled={(state.inv.shiny_lure||0)<=0}>✨ Use Shiny Charm ({state.inv.shiny_lure||0})</button>
        </div>
      </div>
    </div>
  );
